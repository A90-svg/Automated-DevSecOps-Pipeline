 // =====================================
    ...
    // EmailJS configuration is handled server-side for security
    // No tokens should be exposed in client-side code
    ...
    async function sendOtpEmail(targetEmail, code) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          console.error("Failed to send OTP email:", errorMsg, err);
      ...
      if (stored) {
        try {
          ...
          if (parsed.user && !parsed.users) {
            console.log("Migrating to multi-user format...");
            parsed.users = {};
          ...
          console.warn("Failed to parse stored DB, resetting to defaults");
    ...
    document.getElementById("qa-add-funds").addEventListener("click", () => {
      const user = getCurrentUser();
      user.balance = (user.balance || 0) + 50.00;
      saveDB();
      renderBalance();
    });
    document.getElementById("qa-transfer").addEventListener("click", () => {
      const user = getCurrentUser();
      user.balance = (user.balance || 0) - 20.00;
    ...
    document.getElementById("qa-pay-bills").addEventListener("click", () => {
      const user = getCurrentUser();
      user.balance = (user.balance || 0) - 12.300;
      if (!user.transactions) user.transactions = [];
      user.transactions.unshift({
        title: "Utilities",
        note: "Internet",
    ...
    document.getElementById("qa-exchange").addEventListener("click", () => {
      const user = getCurrentUser();
      user.balance = (user.balance || 0) + 5.250;
      if (!user.transactions) user.transactions = [];
      user.transactions.unshift({
        title: "FX Exchange",
        note: "USD to BHD",
